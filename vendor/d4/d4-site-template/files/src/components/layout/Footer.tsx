import Link from "next/link";
import type { CSSProperties } from "react";
import type { NavItem, SocialLink } from "@/types";
import { siteConfig, baseNav, tailNav, socialLinks } from "@/config/site";
import { moduleNav } from "@/config/nav.generated";

const nav: NavItem[] = [...baseNav, ...moduleNav, ...tailNav];
const groups = nav.filter((item) => item.children?.length);
const flat = nav.filter((item) => !item.children?.length);

function SocialIcon({ label }: { label: string }) {
  const name = label.toLowerCase();
  if (name.includes("linkedin")) {
    return (
      <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4" aria-hidden>
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    );
  }
  if (name.includes("facebook")) {
    return (
      <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4" aria-hidden>
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    );
  }
  if (name === "x" || name.includes("twitter")) {
    return (
      <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4" aria-hidden>
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    );
  }
  if (name.includes("instagram")) {
    return (
      <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4" aria-hidden>
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zm0 10.162a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
      </svg>
    );
  }
  if (name.includes("youtube")) {
    return (
      <svg viewBox="0 0 24 24" fill="currentColor" className="h-4 w-4" aria-hidden>
        <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
      </svg>
    );
  }
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="h-4 w-4" aria-hidden>
      <circle cx="12" cy="12" r="9" />
      <path d="M3.6 9h16.8M3.6 15h16.8M12 3a15 15 0 010 18M12 3a15 15 0 000 18" />
    </svg>
  );
}

function FooterColumn({ title, items }: { title: string; items: NavItem[] }) {
  return (
    <div>
      <p className="text-xs font-medium uppercase tracking-widest text-accent">{title}</p>
      <nav className="mt-4 space-y-2.5">
        {items.map((item) => (
          <div key={item.href + item.label}>
            <Link
              href={item.href}
              className="block text-sm text-muted transition-colors hover:text-accent"
            >
              {item.label}
            </Link>
            {item.children?.map((sub) => (
              <Link
                key={sub.href + sub.label}
                href={sub.href}
                className="mt-1.5 block pl-3 text-sm text-muted transition-colors hover:text-accent"
              >
                ↳ {sub.label}
              </Link>
            ))}
          </div>
        ))}
      </nav>
    </div>
  );
}

export default function Footer() {
  return (
    <footer className="border-t border-heading/10 bg-surface">
      <div
        className="mx-auto grid max-w-6xl gap-10 px-4 py-14 sm:grid-cols-2 sm:px-6 lg:grid-cols-[1.4fr_repeat(var(--footer-cols),1fr)_1fr]"
        style={{ "--footer-cols": String(groups.length + 1) } as CSSProperties}
      >
        <div className="max-w-sm">
          <p className="text-lg font-semibold text-heading">{siteConfig.name}</p>
          <p className="mt-2 text-sm leading-6 text-muted">{siteConfig.tagline}</p>
          {socialLinks.length > 0 && (
            <div className="mt-5 flex gap-2">
              {socialLinks.map((s: SocialLink) => (
                <a
                  key={s.href}
                  href={s.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={s.label}
                  className="flex h-8 w-8 items-center justify-center rounded-md border border-heading/15 bg-surface text-muted transition-colors hover:border-accent hover:text-accent"
                >
                  <SocialIcon label={s.label} />
                </a>
              ))}
            </div>
          )}
        </div>

        {groups.length > 0 ? (
          <>
            {groups.map((group) => (
              <FooterColumn key={group.label} title={group.label} items={group.children ?? []} />
            ))}
            <FooterColumn title="Pages" items={flat} />
          </>
        ) : (
          <FooterColumn title="Pages" items={flat} />
        )}

        <div>
          <p className="text-xs font-medium uppercase tracking-widest text-accent">Contact</p>
          <div className="mt-4 space-y-2.5 text-sm text-muted">
            {siteConfig.contactEmail && (
              <a
                href={`mailto:${siteConfig.contactEmail}`}
                className="block break-words transition-colors hover:text-accent"
              >
                {siteConfig.contactEmail}
              </a>
            )}
            {siteConfig.phone && (
              <a
                href={`tel:${siteConfig.phone.replace(/[^\d+]/g, "")}`}
                className="block transition-colors hover:text-accent"
              >
                {siteConfig.phone}
              </a>
            )}
            {siteConfig.address && <p className="leading-6">{siteConfig.address}</p>}
          </div>
        </div>
      </div>
      <div className="border-t border-heading/10 py-4 text-center text-xs text-muted">
        © {new Date().getFullYear()} {siteConfig.name}. All rights reserved.
      </div>
    </footer>
  );
}
